...
function primaryRoutine()
    ...
    while true do
        ...
        if msg == "[PAIRING-REQUEST]" then
            ...
            data.nodes[node] = {}
            saveData()
        elseif msg == "[REMOTE-PAIRING]" then
            if remotePairing then
                send(node,"[REMOTE-PAIRED]")
                remotePairing = false
                data.remotes[node] = true
                saveData()
            end
        elseif msg == "[REMOTE-DATA-REQUEST]" then
            if data.remotes[node] then
                send(node,{data,nodes})
            end
        elseif type(msg) == "table" then
            if msg.command == "[UPDATE-ATTRIBUTE]" and data.nodes[node] then
                ...
                saveData()
            elseif msg.command == "[REMOTE-IO-CHANGE]" and data.remotes[node] then
                log("REMOTE IO UPDATE: "..textutils.serialise(msg.params))
                updateIO(msg.params[1],msg.params[2],msg.params[3])
            end
        end
    end
end
                    ...
                    if tab == "Map" then
                        ...
                        if not hit then
                            if mapObjPlace then
                                ...
                                mapObjPlace = false
                                saveData()
                            elseif mapDrawColor ~= false then
